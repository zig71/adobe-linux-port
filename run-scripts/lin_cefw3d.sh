#!/bin/bash
# CEF wined3d test: builtin d3d11/dxgi for UI Helper, fresh launch, watch.
set -u
...
"$W" regedit /s /tmp/cef_wined3d.reg
echo "applied_rc=$?"
pkill -9 -f 'Creative Clou[d]' 2>/dev/null; sleep 2
cd /home/kubuntu
setsid "$W" 'C:\Program Files (x86)\Adobe\Adobe Creative Cloud\ACC\Creative Cloud.exe' > /tmp/cc-w3d.out 2>&1 < /dev/null &
sleep 25
timeout 40 "$W" 'C:\Program Files (x86)\Adobe\Adobe Creative Cloud\ACC\Creative Cloud.exe' >> /tmp/cc-w3d.out 2>&1
sleep 60
echo "=== CEF tail ==="
...
if [ -n "$WIN" ]; then
  ...
  if [ "$st" = "IsViewable" ]; then
    sleep 15
    spectacle -b -n -o /tmp/cc-w3d.png 2>/dev/null; ls -la /tmp/cc-w3d.png 2>/dev/null
  fi
fi
echo W3D_DONE
